...
export function SidebarFolder({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} disabled={!collapsible} {...props}>
      <FolderContext
        value={useMemo(
          () => ({ open, setOpen, depth, collapsible, hasChildren }),
          [collapsible, depth, open, hasChildren],
        )}
      >
        {children}
      </FolderContext>
...
export function SidebarFolderTrigger({ children, ...props }: CollapsibleTriggerProps) {
  const { open, collapsible, hasChildren } = use(FolderContext)!;
...
export function SidebarFolderLink({
  ...
  const ref = useRef<HTMLAnchorElement>(null);
  const { open, setOpen, collapsible, hasChildren } = use(FolderContext)!;
  const { prefetch } = useSidebar();
